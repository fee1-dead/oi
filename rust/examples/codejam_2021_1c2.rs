...
fn out(number: String) -> Result {
    let mut numbers = vec![];
    let num = number.parse::<u64>()?;

    for concats in 2..=18 {
        let got = first_true(1..=1000000000, |n| {
            let concatenated = (n..n + concats).map(|i| i.to_string()).collect::<String>();
            // dbg!(&concatenated);
            if let Ok(newconcat) = concatenated.parse::<u64>() {
                newconcat > num
            } else {
                true
            }
        ...
        if got != 1000000001 {
            let concatenated = (got..got + concats).map(|i| i.to_string()).collect::<String>().parse::<u64>();
            numbers.extend(concatenated.ok());
        }
    }
