...
fn main() -> Result {
    for i in 1..=get()? {
        ...
        let a = get()?;
        let b = get()?;
        let v = (0..n).map(|_| get()).collect::<Result<Vec<_>, _>>()?;
        out(n, a, b, v);
    }
    Ok(())
}

fn out(_: usize, a: usize, b: usize, v: Vec<usize>) {
    let mut curpos = 0;
    let mut prevconquerred = 0;
    let mut totalcost = 0;
    for number in v {
        if prevconquerred != 0 {
            let costtomove = (prevconquerred - curpos) * b;
            let costtoconqueerfrommove = (number - prevconquerred) * a;
            let movef = costtomove + costtoconqueerfrommove;
            let costtoconquerr = (number - curpos) * a;
            if movef <= costtoconquerr {
                curpos = prevconquerred;
                totalcost += movef;
                prevconquerred = number;
            } else {
                totalcost += costtoconquerr;
                prevconquerred = number;
            }
        } else {
            totalcost += number * a;
