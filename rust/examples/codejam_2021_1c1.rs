...
fn out(numtickets: usize, rangeint: usize, mut ticketssold: Vec<usize>) {
    ticketssold.sort_unstable();

    let mut sol = Vec::new();

    let mut prevticket = None;
    let mut cnt = 0;
    for ticket in ticketssold {
        if let Some(prev) = prevticket.replace(ticket) {
            if ticket - prev > 1 {
                let gap = ticket - prev - 1;
                let ints_to_occupy = (gap + 1) / 2;
                sol.push(ints_to_occupy);
                sol.push(gap - ints_to_occupy);
                cnt += 1;
            }
        } else {
            let gap = ticket - 1;
    ...
    let prevticket = prevticket.unwrap();
    let rangediff = rangeint - prevticket;
    sol.push(rangediff);

    sol.sort_unstable();

    let ints: usize = sol.into_iter().rev().take(2).sum();

    let prob = ints as f64 / rangeint as f64;
    println!("{}", prob);
}