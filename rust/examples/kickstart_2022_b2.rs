...
fn out(n: usize) {
    let numdigits = (n as f64).log10() as u32;
    let mut cnt = 0;

    for digits in 1..=(numdigits / 2 + 1) {
        for palin in 10usize.pow(digits - 1)..10usize.pow(digits) {
            for realdigits in digits * 2 - 1 ..= digits * 2 {
                let mut palin = palin;
                let mut newhalf = palin;
                if realdigits % 2 == 1 {
                    palin /= 10;
                }
                while newhalf > 0 {
                    palin = palin * 10 + newhalf % 10;
                    newhalf /= 10;
                }
                if n % palin == 0 {
                    cnt += 1;
