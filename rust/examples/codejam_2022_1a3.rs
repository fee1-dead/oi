...
fn main() -> Result {
    for i in 1..=get()? {
        ...
        for _ in 0..a {
            let mut vp = Vec::with_capacity(b);
            for _ in 0..b {
                vp.push(get()?);
            }
            v.push(vp);
...
memoize! {
    fn intersection[I](l: usize, r: usize, @( weights: &[Vec<isize>] )) -> isize {
        let weight_count = weights[0].len();
        (0..weight_count).map(|i| weights[l..=r].iter().map(|weights| weights[i]).min().unwrap()).sum()
    }
}

memoize! {
    fn minimum[M](l: usize, r: usize, @( weights: &[Vec<isize>] )) -> isize {
        if l == r { return 0 }
        (l..r).map(|midpoint| {
            minimum(l, midpoint, weights) + minimum(midpoint + 1, r, weights) + 2 * (
                intersection(l, midpoint, weights) + intersection(midpoint + 1, r, weights) -
                2 * intersection(l, r, weights)
            )
        }).min().unwrap()
    }
}

fn out(_: usize, _: usize, v: Vec<Vec<isize>>) {
    I.with(|r| r.borrow_mut().clear());
    M.with(|r| r.borrow_mut().clear());
    println!("{}", minimum(0, v.len()-1, &v) + 2 * intersection(0, v.len()-1, &v));
}
