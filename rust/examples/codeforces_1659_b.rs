...
fn out(_: usize, mut k: usize, s: String) {
    let default_keep = k % 2 == 0;
    let mut newnums = Vec::with_capacity(s.len());
    let mut news = String::new();
    for c in s.chars() {
        if c == '0' && default_keep && k > 0 {
            k -= 1;
            newnums.push(1);
            news.push('1');
            continue;
        }
        if c == '1' && !default_keep && k > 0 {
            k -= 1;
            newnums.push(1);
            news.push('1');
            continue;
        ...
        if default_keep {
            ...
            news.push(if c == '0' { '1' } else { '0' });
        }
    }

    if k > 0 {
        *newnums.last_mut().unwrap() += k;
        if k % 2 != 0 {
            if news.pop().unwrap() == '0' {
                news.push('1')
            } else {
                news.push('0');
            }
    ...
    println!("{news}");
    for (i, c) in newnums.into_iter().enumerate() {
        if i != 0 { print!(" ") }
        print!("{c}")
    }
    println!()
