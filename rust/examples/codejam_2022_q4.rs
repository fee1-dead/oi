...
fn solve() -> Result<()> {
    ...
    fn dfs_min(node: usize, fun: &[u64], minimum: &mut [u64], pointed_to_by: &[Vec<usize>]) {
        ...
        minimum[node] = if ret == std::u64::MAX { fun[node] } else { fun[node].max(ret) };
    }
    ...
    fn dfs_solve(node: usize, fun: &[u64], minimums: &mut [u64], pointed_to_by: &[Vec<usize>], total: &mut u64, add: bool) {
