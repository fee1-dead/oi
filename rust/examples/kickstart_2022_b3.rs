...
fn main() -> Result<()> {
    for i in 1..=get()? {
        ...
        print!("Case #{}: ", i);
        out((0..n).map(|_| get()).collect::<Result<_, _>>()?, d);
    }
    Ok(())
}

fn out(mut v: Vec<i64>, d: i64) {
    v.dedup();
    let mut minsum = std::i64::MAX;
    for (i, mut number) in v.iter().copied().enumerate() {
        let mut l = i;
        let mut r = i;
        let mut sum = 0;
        loop {
            ...
            let mindiff = |a: i64, b: i64| {
                let absdiff = (a - b).abs();
                absdiff.min(d - absdiff)
            };
            let choice = match choices[..] {
                [a, b] => {
                    if mindiff(v[a], number) < mindiff(v[b], number) {
                        l -= 1;
                        a
                    } else {
                        r += 1;
